...
%% \todo{Do we need all this detail?}
%% the vertical bars suggest we have lots of day-job developers, will point this out below
...
\Cref{t:session-size} outlines the size of sessions using two metrics:
...
If a creator removes one \code{NotATable} error but then
...
\begin{itemize}
  ...
  \item
    ...
    \code{UnknownProperty} are the most popular.
    ...
    \code{bucket.countFish()} letter-by-letter will see errors
    including \code{UnknownSymbol(buck)} and \code{UnknownProperty(bucket.c)}.
...
\begin{figure}[t]\centering
  ...
  \begin{tabular}{lll}
    ...
    \includegraphics[width=0.24\columnwidth]{img/compass-nocheck.pdf}
    &
    \includegraphics[width=0.24\columnwidth]{img/compass-nonstrict.pdf}
    &
    \includegraphics[width=0.24\columnwidth]{img/compass-strict.pdf}
  \end{tabular}
  \caption{Comparing the change in type errors to the change in \FS{} errors.}
  \label{f:tefs-compass}
\end{figure}

\Cref{f:tefs-compass} takes a closer look at type errors (\tekey{}) and \FS{} errors (\fskey{}).
...
\begin{itemize}
  \item
    All three plots have a nonzero percentage of \code{te only} items.
  ...
  \item
    ...
    half the records are \code{zero}, nearly half are \code{bg only}, and only a few
    land in the other categories.
    While the number of zeros is a bit high, the high \code{bg only} and low
    \code{agree} counts make sense because \mnocheck{} reports few errors.

  \item
    In \mnonstrict{}, the number of agreements jumps up to nearly \pct{20}.
