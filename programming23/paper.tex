...
% TODO BEFORE FINAL
% - update all refs to artifacts to no longer be in future tense

% The paper on using Luau telemetry to measure the effectiveness of type error reporting.

\documentclass[english,cleveref]{programming}
...
\title{Privacy-Respecting Type Error Telemetry at Scale}
...
postdoc at Brown University and is now an assistant professor in the
...
\begin{abstract}
  ...
  As part of its efforts to support all kinds of programmers, Luau includes an
  ...
  Since Luau is currently being used by many creators, we want to collect data
  to improve the language and, in particular, the type system.
  The standard way to collect data is to deploy client-side telemetry; however, we cannot
  ...
  We designed and implemented a pseudonymized, randomly-sampling telemetry system for Luau.
...
70~million Daily Active Users, and 16~billion hours of engagement in
July--September 2023~\cite{corp.roblox.com}.
...
The \mnocheck{} mode reports only syntax errors,
the \mnonstrict{} mode reports would-be runtime errors and skips over
other issues,
and the \mstrict{} mode reports a variety of potential errors.
In addition, no matter the mode, a {\FS{}} type analysis runs to guide \IDE{} tools~(\cref{s:context}).
...
\begin{itemize}
  ...
  \item
    ...
    and scripts to analyze them: \url{https://zenodo.org/doi/10.5281/zenodo.10275213}
...
\begin{table}
  ...
  \begin{tabular}{ll}
    ...
    \code{CodeTooComplex} & Type analysis failed~(\cref{s:code-too-complex}). \\
    %% \code{UnificationTooComplex} & Type analysis failed, unification solver hit a limit \\
    \code{GenericError}
    & Generic label for other non-type errors,\\
    & \hbox{}~~such as looping over an unordered table.
...
Imports therefore form a graph in general, though the analysis
removes edges to ensure an acyclic graph.
In \mstrict{} mode, it is an error to create an import cycle.
...
When typechecking is performed for autocomplete, the common
case is that only the current script gets checked because it
is the only dirty script.
(Since the module graph is acyclic, nothing that the current
script requires can transitively require it.)
...
\begin{enumerate}
  ...
  \item
    Telemetry \emph{must transmit a small amount of data} to avoid overloading the servers.
    This constrains both the number of records the system can transmit and the size of each record.

\end{enumerate}
See Cox~\cite{transparent-telemetry} for further discussion of
the privacy implications and tradeoffs in telemetry.
...
\begin{itemize}
  \item
    Lifecycle events including save, run, quit, and publish.
    {(Publish events
...
This means that it cannot send event-based telemetry on IDE lifecycle events
(due to Studio's architecture) and it cannot send full results from every type
analysis because that would overwhelm the data-collection server.
...
\begin{enumerate}
  \item
    \uid{}: a \panon{} identifier (15-digit random number) for the current session.
  \item
  ...
  \item
    Overall Counts: summary of type errors and \FS{} errors during the
    last two invocations of type analysis, and the number
    ...
    \end{itemize}
  \item
    Edit Range Counts: list of specific errors that arose in the last two
...
telemetry records because there is only one interval for errors to overlap with.

This telemetry design is admittedly coarse-grained.
For instance, it does not distinguish between edits that
ignore a type error from edits that remove one error
while introducing another.
We acknowledge this and other threats in~\cref{s:threats}.
The main advantages of this telemetry are the small size of each record and
the complete lack of private information.
...
For each of the 24 hours in each day, there is one blue bar
indicating the number of records generated during that hour.
Each hour has a few dozen to over 3,000 records, typically about
1,000.
We conjecture the very low bars are due to outage on
the logging server and that the very tall bars are due to
user scripts that generated a huge number of keystrokes.
%% two biggest towers (7050 6691) are mostly from 2 sessions:
%% (("\"178,020,742,609,772\"" . 6079))
%% (("\"458,983,252,701,986\"" . 5085))
Telemetry ended in early April; however, there are short bars in the mid-April
range from developers who waited a few days to update Roblox Studio.
...
\begin{table}[t]\centering
  \caption{Size of analyzed code: number of lines and lines in edit range}
...
% 51,761 &              &
  \begin{tabular}[t]{lrrrrl}
    ~          & Mean  & Stddev & Median & \ptile{99} & Distribution \\\midrule
    ...
    %% TODO x-axis starts at 1, not zero ... do we have zeros?! may need to explain
  \end{tabular}
\end{table}
...
There is a huge amount of variation across sessions.
The largest ones have over 50,000 lines of code
while the smallest have zero lines of code.
%% TODO filter the zero lines? what happens in that data?? run a query
...
%% power law test~\cite{csn-siamr-2009,g-jss-2015}
%%  lines     xmin = 2 alpha = 1.46 p = 0.1 (low!)
%%  editrange xmin = 1 alpha = 1.48 p = 0.73
...
%% power law test~\cite{csn-siamr-2009,g-jss-2015}
%%  timespan    xmin = 1 alpha = 2.88 p = 0.23
%%  event-count xmin = 1 alpha = 1.33 p = 0.86
...
and a small fraction of these overlapped with the current edit range~(\pct{5} for
type errors, \pct{1} for \FS{}).
The other errors point to different modules in the codebase.

The fraction of errors in the current module is relatively high because projects
contain many modules, all of which might contain latent type errors.
This high fraction is encouraging for two reasons.
...
module.
Second, it suggests that creators fix errors before switching
to another module.

The low fraction of errors in the edit range sends a mixed message:
\begin{itemize}
  \item
    On one hand, it may be that edits successfully remove errors.
  ...
  \item
    ...
    While non-privacy-protecting analyses have answered such
...
Among the other sessions, most of those (\pct{80}) switch to a
...
This data excludes module switch records and considers only keystroke events.
...
errors.
This is surprising;~\cref{s:tefs-compass} explores further.
...
\begin{itemize}
  \item
    ...
    \Cref{s:count-analysis-errors} discusses the implications
    of this low overlap rate for future work.

  \item
    \code{SyntaxError} and the related errors \code{UnknownSymbol} and
    \code{UnknownProperty} are the most common.
    If the edit range errors are indicative of type errors at large,
    then most errors in the data are likely due to typos during edits.
    These errors also survive (\keepsym{}) the more than any other.

    Curiously, the number of syntax errors never decreases or stays the same.
    This may be because the easy fix is to press undo, and telemetry cannot
    see undo events.

    %% TODO awkward
    Our telemetry strategy likely inflates the number of typos because it cannot
    determine when a creator is mid-edit.
    ...
    \code{UnknownProperty} for \code{bucket.c}, which may end up getting sampled.
...
\Cref{t:type-error-count} sorts type errors by
...
\begin{itemize}
  \item
    Typo-level errors, in particular \code{UnknownSymbol} (for an unbound variable),
  ...
  \item
    ...
    never appeared in \mstrict{} records:
    \code{DuplicateTypeDefinition}, \code{FunctionDoesNotTakeSelf},
    \code{MissingUnionProperty}, and \code{OnlyTablesCanHaveMethods}.
...
Hitting a limit triggers one of the following errors along with a message
that the analysis failed to understand the code:
\code{CodeTooComplex},
...
\begin{figure}[t]\centering
  ...
  \mnocheck{}
  \includegraphics[width=\columnwidth]{img/error-count-nocheck-row--te-density-diff-no-switch.pdf}

  \mnonstrict{}
  \includegraphics[width=\columnwidth]{img/error-count-nonstrict-row--te-density-diff-no-switch.pdf}

  \mstrict{}
  \includegraphics[width=\columnwidth]{img/error-count-strict-row--te-density-diff-no-switch.pdf}
  \caption{Changes in type error density ($\frac{\mbox{\#curr - \#old errors}}{\mbox{\#lines}}$) over time (seconds).}
...
The data excludes module-switch telemetry records to keep the focus on edits.
...
ends at 150 seconds to show some but not all of the long tail.
...
(max \mnocheck{}: 51, max \mnonstrict{}: 3586, max \mstrict{}: 87), we focus
on $\pm{}50$ errors because most of the mass is in that region.
The points are color-coded by their session.
There are many colors througout the plot, which shows that the data comes
from many different sessions rather than from a few high-traffic sessions.
...
\begin{itemize}
  ...
  \item
    Only a small fraction of \mnocheck{} records have nonzero type error density.
    This is in line with~\cref{t:type-error-survival}.
    Module switches in \mnocheck{} have nonzero density more often, but still
    rarely (\pct{3}).
    %% 34372 / 1337552 = 100 * 0.025664048949124968
  ...
  \item
    \mnonstrict{} has a bigger fluctuations in errors than \mstrict{}.
...
\begin{figure}[t]\centering
  ...
  %% with NON-moduleswitch data
  % TE nocheck & 961 & (\pct{38.50}) & 694 & [\pct{27.80}] & 841 & [\pct{33.69}] \\
  % TE nonstrict & 8623 & (\pct{37.67}) & 5263 & [\pct{22.99}] & 9004 & [\pct{39.34}] \\
  % TE strict & 442 & (\pct{38.27}) & 244 & [\pct{21.13}] & 469 & [\pct{40.61}] \\

  % TE mod nocheck & 0 & (\pct{na}) & 0 & [\pct{na}] & 0 & [\pct{na}] \\
  % TE mod nonstrict & 7185 & (\pct{36.61}) & 4768 & [\pct{24.30}] & 7672 & [\pct{39.09}] \\
  % TE mod strict & 251 & (\pct{35.35}) & 204 & [\pct{28.73}] & 255 & [\pct{35.92}] \\

  % FS nocheck & 156422 & (\pct{37.19}) & 153965 & [\pct{36.61}] & 110194 & [\pct{26.20}] \\
  % FS nonstrict & 18927 & (\pct{37.41}) & 17791 & [\pct{35.17}] & 13872 & [\pct{27.42}] \\
  % FS strict & 296 & (\pct{31.66}) & 360 & [\pct{38.50}] & 279 & [\pct{29.84}] \\

  % FS mod nocheck & 153913 & (\pct{36.96}) & 152735 & [\pct{36.68}] & 109807 & [\pct{26.37}] \\
  % FS mod nonstrict & 18573 & (\pct{37.09}) & 17604 & [\pct{35.15}] & 13905 & [\pct{27.76}] \\
  % FS mod strict & 272 & (\pct{32.04}) & 317 & [\pct{37.34}] & 260 & [\pct{30.62}] \\

  \begin{tabular}{l@{~~}lr@{}rr@{}rr@{}r}
    & & \zerowidth{\rbox{\addsym{}}} & & \zerowidth{\ybox{\keepsym{}}} & & \zerowidth{\gbox{\dropsym{}}} \\\midrule
    type error  & \mnocheck{}   &      0 & [\nanpct{}  ] &      0 & [\nanpct{}]   &      0 & [\nanpct{}] \\
    (in module) & \mnonstrict{} &   7185 & [\pct{36.61}] &   4786 & [\pct{24.30}] &   7672 & [\pct{39.09}] \\
                & \mstrict{}    &    251 & [\pct{35.35}] &    204 & [\pct{28.73}] &    255 & [\pct{35.92}] \\
    \\[-2ex]
    \FS         & \mnocheck{}   & 153913 & [\pct{36.96}] & 152735 & [\pct{36.68}] & 109807 & [\pct{26.37}] \\
    error       & \mnonstrict{} &  18573 & [\pct{37.09}] &  17604 & [\pct{35.15}] &  13905 & [\pct{27.76}] \\
    (in module) & \mstrict{}    &    272 & [\pct{32.04}] &    317 & [\pct{37.34}] &    260 & [\pct{30.62}] \\
  \end{tabular}
...
\begin{itemize}
  \item
    None of our \mnocheck{} samples captured a change in syntax errors in the
    current module.
    The nonzero changes in density above~(1802 total, shown in \cref{f:error-density}) all affected
    other modules.
    %% TODO doublecheck really zero??!
    %% QALAN what would this mean ... how can one module change stx errors in another??
    %% 961 + 841 = 1802

  \item
    The type error increases and decreases are evenly balanced in
    \mnonstrict{} and \mstrict{} modes ($\pm\pct{3}$).
    Since this data is from keystrokes rather than module switches,
    it appears that sessions tend to fix errors highlighted by type analysis.

  \item
    \mstrict{} mode has a higher percent of ``maintain'' (\keepsym{}) records.
    Errors may be more likely to persist in this mode.
    %% NOTE these "=" are all maintaining a nonzero number of errors

  \item
    In \FS{} mode, the percent of ``maintain'' records is higher across
    the board and roughly equal to the percents of increases and decreases
...
\pct{16} of all \mstrict{} records increase (\addsym{}) the number of type errors
but maintain (\keepsym{}) the number of \FS{} errors.
%% 16% = from code/compass.rkt
...
\begin{description}
  \item[RQ1.]\leavevmode
    \begin{description}
      \item[- How many sessions use type analysis?]
        ...
        But, few sessions use type analysis directly.
      ...
      \item[- How often do projects contain modules with different modes?]
        Sessions rarely combine analysis modes.
        Only 512 of \nK{340} sessions ($<\pct{0.15}$) switched to modules with different
        analysis modes.
        This number may be an underapproximation because we have data
        ...
        If $\pct{90}$ of \emph{all} modules indeed use \mnocheck{}, then multi-mode
        projects must be rare.
      ...
      \item[- How often do sessions turn analysis off?]
        ...
        Then again, sessions rarely turn analysis on as well:
        only 176 ($<\pct{0.05}$) switch to a stronger mode.
  ...
  \item[RQ2. For modules that use type analysis (\mstrict{} and \mnonstrict{}):]\leavevmode
    \begin{description}
...
%%   (SyntaxError "0.01")
  ...
  \item[RQ3. What impact does type analysis have on the number of \FS{} errors?]
    ...
    A randomly-chosen edit has a roughly 1/3 chance of increasing, decreasing, or
...
of these actions.
We have no access to other events, e.g., local runs~(\cref{s:tel-limitations});
...
On a related note, the computatation that determines whether an old
type error overlaps with the current edit range has many edge cases,
especially to handle deletions, and may mis-count.
This could explain why~\cref{t:type-error-survival} reports zero syntax errors
that overlap with the current edit range; if the fix is to delete code,
then the edit range is empty.
...
VSCode~\cite{vscode-telemetry}, IntelliJ~\cite{intellij-telemetry},
\code{.NET}~\cite{dotnet-telemetry}
and
Eclipse~\cite{eclipse-udc},
...
Recent work shows how to add noise to traces~\cite{zhlbr-cc-2020},
...
Roblox Studio could benefit from these techniques.
...
Prio and Poplar demonstrate a telemetry method to compute public aggregates
over private data using a middle layer of
servers~\cite{cb-nsdi-2017,bbcgi-sp-2021}.
Clients can depend on privacy guarantees even if other clients and some
servers (but not all) are malicious.
The design has been deployed in Mozilla Firefox to monitor blocklist
usage~\cite{mozilla-prio}.
Luau telemetry might benefit from a redesign to use middle-layer servers,
though it is unclear what private data from type analysis would support meaningful
aggregates.
%% In particular, the data would still need to be relatively small to trasmit.

Cooperative bug isolation is a method for designing
telemetry systems~\cite{liblit-thesis}.
The goal is to collect a small amount of data from each user and perform
statistically-sound analyses; privacy is not a main concern.
...
There is a rich body of work that uses IDE instrumentation to study programmers
(e.g.,~\cite{pgnwk-koli-2022,wk-icer-2019,hl-icse-2010,j-ieeesoft-2013,kje-ase-2010, mmlb-swvis-2014, ncvjd-ecoop-2013, rl-icse-2008, we-xp-2004,bgimgm-cse-2016,dlc-iticse-2014}).
The Blackbox project has accumulated terabytes of Java code over a ten-year span,
and has thereby enabled dozens of contributions to CS education~\cite{bask-icer-2018,bkmu-sigcse-2014}.
The GRUMPS telemetry system collected over four million keystroke-level actions to study
student programmers~\cite{tkdmceg-ascilite-2003,tm-iticse-2004}.
WatchDog records the time that programmers spend reading, writing, and testing
code~\cite{blpgz-icse-2016}.
TDD-Guide collects similar data to teach testing methods~\cite{mdk-xp-2008}.
None of the above are sufficiently private to work for Roblox Studio.
The BitFit project counts notable events (compile program, ask for hint, etc.)
for Java~\cite{ekc-wccce-2016,anna-russo-kennedy-ms-2006}.
While it does operate in a transparent~\cite{transparent-telemetry} manner, it
provides only coarse-grained information that would be insufficient in our
study.
...
interviews~\cite{cdhhjklwya-hatra-2020,cams-oopsla-2020,gstf-hatra-2021},
or static
corpuses~\cite{bhmvv-toplas-2019,rppd-fse-2014}.
Controlled experiments are rare;
Kleinschmager \etal{}~\cite{khrts-icpc-2012} is a notable exception.
Lerner \etal{}~\cite{lfgc-pldi-2007} apply a type error repair tool on
over two thousand programs.
Seidel \etal{}~\cite{sjw-jfp-2018,sscwj-oopsla-2017} use five thousand programs
...
an industrial strength typechecker in a privacy-respecting way.
Despite the intentionally-restricted data,
the analysis offers several lessons for Luau in
particular and for other gradual languages~(e.g.~\cite{bat-ecoop-2014,cdv-elixir-types-2023,tfffgksst-snapl-2017}) in general:
\begin{itemize}
  \item
    ...
    other modes exist.
    Luau, and perhaps other languages, must take care to advertise gradual
    types and demonstrate how to use them effectively to increase adoption.
%  \item
...
%    \pct{5} of (current? old??) type errors overlap with edit range, so they don't stick around.
  \item
    ...
    Other languages should consider adding a nonstrict mode that minimizes false positives
    as a bridge toward strict type checking.
  ...
  \item
    ...
    Other languages should avoid making the same mistake.
\end{itemize}
...
\begin{itemize}
  ...
  \item
    ...
    that error deltas bounce up and down rather than rising sharply, then falling~(\cref{f:error-density}).
\end{itemize}

One aspect of the telemetry that we would change for next time is to provide
more metadata.
For example, Luau recently adopted semantic subtyping to reduce false-positive type
...
\begin{enumerate}
  ...
  \item It would be useful to track the identity of errors (to know exactly which ones get fixed),
    to have fine-grained counts for \FS{} analysis errors, and to
    record the specific reason behind errors such as \code{TypeMismatch}.
    But, this additional data could easily double the size of telemetry records.
...
NSF grant \href{https://www.nsf.gov/awardsearch/showAward?AWD_ID=2227863&HistoricalAwards=false}{SHF-2227863}
and NSF grant \href{https://nsf.gov/awardsearch/showAward?AWD_ID=2030859&HistoricalAwards=false}{2030859}
...
% \begin{itemize}
%   \item
%     A \code{"-"} entry denotes a zero.
%     This data was not in the telemetry record.
%     Kibana filled in the dash.
%   \item
%     Timestamps are California local time.
%     %% (Daylight savings?)
%   \item
%   \item
...
% 
% \begin{figure}[t]\centering
%   %% +-200 background errors, same as ever! balanced
%   \mnocheck{}
%   \includegraphics[width=\columnwidth]{img/error-count-nocheck-row--fs-density-diff.pdf}
% 
%   \mnonstrict{}
%   \includegraphics[width=\columnwidth]{img/error-count-nonstrict-row--fs-density-diff.pdf}
% 
%   \mstrict{}
%   \includegraphics[width=\columnwidth]{img/error-count-strict-row--fs-density-diff.pdf}
%   \caption{Changes in \FS{} error density (errors / lines) over time (seconds).}
%   \label{f:error-density}
% \end{figure}
...
% 
%   \mnocheck{}
%   \includegraphics[width=\columnwidth]{img/error-count-nocheck-row--te-density-diff-yes-switch.pdf}
% 
%   \mnonstrict{}
%   \includegraphics[width=\columnwidth]{img/error-count-nonstrict-row--te-density-diff-yes-switch.pdf}
% 
%   \mstrict{}
%   \includegraphics[width=\columnwidth]{img/error-count-strict-row--te-density-diff-yes-switch.pdf}
%   \caption{Changes in type error density ($\frac{\mbox{\#errors}}{\mbox{\#lines}}$) over time (seconds).}
%   \label{f2:error-density}
% \end{figure}
% 
% \begin{figure}[t]\centering
%   ALL
% 
%   \mnocheck{}
%   \includegraphics[width=\columnwidth]{img/error-count-nocheck-row--te-density-diff-all.pdf}
% 
%   \mnonstrict{}
%   \includegraphics[width=\columnwidth]{img/error-count-nonstrict-row--te-density-diff-all.pdf}
% 
%   \mstrict{}
%   \includegraphics[width=\columnwidth]{img/error-count-strict-row--te-density-diff-all.pdf}
%   \caption{Changes in type error density ($\frac{\mbox{\#errors}}{\mbox{\#lines}}$) over time (seconds).}
%   \label{f3:error-density}
% \end{figure}
